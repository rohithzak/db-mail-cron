...
pg_dump -U postgres -d db_name > /backup/$filename 
	if [ $? -eq 0 ]; then
		...
		/usr/bin/python3 mail.py <sender mail ID> FAILED "SQL dump unsuccessful for hotcri"  # <sender mail ID> FAILED and sql sump unsuccessful are my parmaeter of choice, you can modify as per your requirement
		exit 1
        fi                                                             #Checks if the backup-process is completed successfully and sends mail in failure and push to s3 in success

/usr/bin/python3 mail.py <Sender mail ID> SUCCESSFUL "Pushed to S3 Successfully"  #After pushing to s3, sends mail with success message
